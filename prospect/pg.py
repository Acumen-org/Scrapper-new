...
def _convert_ddl(sql: str) -> str:
    ...
    # SQLite INTEGER is 64-bit; Postgres integer is 32-bit and overflows on
    # any dollar amount above ~2.1 billion, which RAUM and fund GAV routinely
    # exceed. _INT_PK above has already rewritten the identity columns, so this
    # only widens the plain ones.
    sql = _PLAIN_INTEGER.sub("BIGINT", sql)
...
def translate(sql: str, has_params: bool = False) -> str | tuple[str, tuple]:
    ...
    sql = _GROUP_CONCAT_2.sub(r"STRING_AGG(\1, \2)", sql)
    sql = _GROUP_CONCAT_1.sub(r"STRING_AGG(\1, ',')", sql)
...
class Cursor:
    ...
    def _rollback_on_error(self):
        """Postgres aborts the entire transaction on any failed statement.
        SQLite did not, and several read paths here catch-and-continue on the
        assumption that a failed query is survivable -- a missing table behind
        a badge count, say. Without this the next statement on the connection
        fails too, and a degraded count takes the whole page down instead.
        The exception is still raised; only the poisoned transaction is undone.
        """
        try:
