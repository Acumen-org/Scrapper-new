...
REVIEW_CSS = """
.sent{background:var(--side);border-radius:8px;padding:8px 11px;font-size:13px;
margin-top:6px;font-family:Georgia,serif;font-style:italic}
  ...
  <button name="decision" value="denied">Different firm</button></form></td></tr>""")
    ...
    if kind in ("", "brochure_negation"):
        negs = c.execute(f"""
            ...
            LIMIT ? OFFSET ?""",
            (per, (page_n - 1) * per) if kind == "brochure_negation" else (per, 0)).fetchall()
        total += n_neg
        for n in negs:
            rows_html.append(f"""<tr>
<td><span class="chip warn">Negation</span></td>
<td><a href="/firm/{esc(n['crd'])}">{escn(n['legal_name'] or '')}</a>
...
<p class="meta" style="margin:0 0 12px">A confirmed 13F match feeds holdings on the next run;
a wrong one would put someone else&rsquo;s holdings in a call opener, which is why nothing
merges without a person. An unreviewed negation never removes a tag, it only holds its
confidence lower. Each decision re-scores the firm at once.</p>
...
<th style="width:220px">Decision</th></tr></thead>
<tbody>{''.join(rows_html) or
'<tr><td colspan="5" class="empty">Queue is empty.</td></tr>'}</tbody>
</table>
{pager_html}
</div>"""
